...
function checkInstalledNodeModules(root, findings) {
  const axiosPkg = path.join(root, 'node_modules', 'axios', 'package.json');
  if (fs.existsSync(axiosPkg)) {
    let version = '';
    try {
      version = String(JSON.parse(safeRead(axiosPkg) || '{}').version || '').trim();
    } catch (_err) {
      version = '';
    }
    if (MALICIOUS_AXIOS_VERSIONS.has(version)) {
      findings.push(
        finding('critical', 'installed-package', axiosPkg, `Installed axios version is malicious: ${version}`, 'Known compromised version discovered directly in node_modules.')
      );
    }
  }

  const plainCryptoDir = path.join(root, 'node_modules', MALICIOUS_DEPENDENCY);
  if (fs.existsSync(plainCryptoDir)) {
    ...
    let sev = 'high';
    const pkg = path.join(plainCryptoDir, 'package.json');
    if (fs.existsSync(pkg)) {
      try {
        const version = String(JSON.parse(safeRead(pkg) || '{}').version || '').trim();
